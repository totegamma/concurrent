...
投稿されるコンテンツはすべて署名付きドキュメントとして表現されるため、どのサーバーに保存されていても、それが本当に本人のものであることを誰でも検証できます。
サーバーには一切機密情報を渡さず、アカウントのセキュリティはユーザー自身が責任を持って管理します。

## 特徴:
- **プロトコルレベルで設計された引っ越し機能** — あなたのアイデンティティは鍵ペアそのものであり、どこかのサーバーのデータベースの1行ではありません。署名付きドキュメントの全ログをエクスポートし、新しいサーバーで再生するだけで、投稿もつながりもそのままにアカウントを再開できます。
- **サーバーを越えて参加できるコミュニティタイムライン** — 各サーバーに話題ベースのコミュニティタイムラインを好きな数だけ作成できます。単一サーバー内に閉じた「ローカルタイムライン」とは異なり、他のサーバーのユーザーも閲覧・参加できます。
- **小さな仕様を組み合わせたプロトコル** — プロトコルは[CIPs (Concrnt Improvement Proposals)](https://github.com/concrnt/CIPs-translated)と呼ばれる小さく独立した仕様の集まりとして定義されています。第三者が再実装しやすいよう意図的にシンプルに保たれており、静的ファイルホスト+小さな書き込み処理(サーバーレス関数でも可)という最小構成のサーバーすら成立します。
- **プログラマブルな権限管理** — アクセス制御はポリシーエンジンで表現されます。決められた公開範囲の選択肢から選ぶのではなく、誰が読めるか・書けるか・参加できるかを、コミュニティやユーザーが細かくカスタマイズできます。
- **モジュールによる拡張** — 追加機能は独立したサービスとして動作し、実行時にサーバーへ登録されます。コアをフォークすることなくサーバーの機能を拡張できます。
...
# マニア向け情報
## 自鯖の建て方やプロトコル仕様等
詳細なドキュメントサイトはこちら: [concrnt square](https://square.concrnt.net/getting-started/hosting/)

ローカルでフルスタック(サーバー + Postgres + Redis + memcached + web UI)を試すには:
...
## プロトコル仕様
プロトコルは[CIPs (Concrnt Improvement Proposals)](https://github.com/concrnt/CIPs-translated)と呼ばれる小さなドキュメントの集まりとして仕様化されています。

## Contributing
PRを作成する場合は基本的に先にissueを作成し、その可否のコンセンサスを取ることをお勧めします。(Concrntは現在heavilyに開発されており、その方針上入れられない変更であることがあります。)

---

> [!IMPORTANT]
> v1.10.0から仕様が大きく更新されています。以前のバージョンのサーバーを利用している方は、[マイグレーションガイド](https://square.concrnt.net/operator/migration/)を参考にマイグレーションを行ってください。
