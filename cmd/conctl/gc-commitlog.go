...
var gcCommitlogCmd = &cobra.Command{
	...
	Long: "When a record key is overwritten, the superseded document's commit log is kept\n" +
		...
		"captured old document a no-op. Once the document's createdAt has fallen out of\n" +
		"the backdate window a replay is rejected as too old anyway, so the tombstone is\n" +
		"redundant and the row can be deleted (commit_owners rows cascade with it).\n" +
