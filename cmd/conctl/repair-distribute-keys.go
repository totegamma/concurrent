...
var repairDistributeKeysCmd = &cobra.Command{
	...
	Long: "Distribute references (CIP-7) used to be keyed <destination>/<time-based CDID of the\n" +
		...
		"each old-rule row (schema reference.json, document-reference proof, 26-char time-CDID\n" +
		"key segment) to its new-rule key, recomputing the document id since the key is part of\n" +
		"the document. If a new-rule row already exists for the same key (a post-deploy\n" +
		"overwrite recreated it), the old-rule row is simply dropped in its favor.\n" +
	...
	RunE: withOperationContext(func(cmd *cobra.Command, args []string, op *operationContext) error {
		...
		// here; the document's own schema is checked below
		...
		if err := op.DB.WithContext(ctx).
			...
			return fmt.Errorf("failed to query reference record keys: %w", err)
		}
		...
		for _, rk := range keys {
			...
			// old rule = the original document's time-based CDID (26 chars);
			// anything else — hash-CDID rows already on the new rule, or a
			// user-committed record that merely shares the schema — is not ours
		...
		fmt.Fprintf(os.Stderr, "%s %d of %d reference records\n", action, repaired, len(keys))
		return nil
	}),
}
