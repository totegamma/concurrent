...
func isNoSuchEntity(err error) bool {
	return errors.Is(err, gcdatastore.ErrNoSuchEntity)
}
...
func (s *store) getCommit(ctx context.Context, id string) (commitLogModel, error) {
	...
	err := s.client.Get(ctx, s.key(kindCommitLog, id), &commit)
	if err != nil {
...
func (s *store) getRecord(ctx context.Context, id string) (recordModel, error) {
	...
	err := s.client.Get(ctx, s.key(kindRecord, id), &record)
	if err != nil {
		if isNoSuchEntity(err) {
...
func (s *store) getRecordKey(ctx context.Context, uri string) (recordKeyModel, error) {
	...
	err := s.client.Get(ctx, s.key(kindRecordKey, uri), &rk)
	if err != nil {
		if isNoSuchEntity(err) {
...
func (s *store) createCommitLogAndOwners(ctx context.Context, tx *gcdatastore.Transaction, commit usecase.CommitWrite) (time.Time, error) {
	commitKey := s.key(kindCommitLog, commit.ID)
	var existing commitLogModel
	err := tx.Get(commitKey, &existing)
	now := time.Now().UTC()
	commitCreatedAt := now
	if err == nil {
		...
		if _, err := tx.Put(commitKey, &newCommit); err != nil {
	...
	for _, owner := range commit.Owners {
		ownerKey := s.key(kindCommitOwner, compoundKey(owner, commit.ID))
		var existingOwner commitOwnerModel
		err := tx.Get(ownerKey, &existingOwner)
		if err == nil {
			continue
		}
		if !isNoSuchEntity(err) {
		...
		if _, err := tx.Put(ownerKey, &commitOwner); err != nil {
			return time.Time{}, err
		}
	}
...
func (s *store) ensureParentRecordKeys(ctx context.Context, tx *gcdatastore.Transaction, uri string) (string, error) {
	...
	key := s.key(kindRecordKey, parentURI)
	...
	if err == nil {
		...
		if changed {
			if _, err := tx.Put(key, &existing); err != nil {
				return "", err
			}
		}
	...
	if _, err := tx.Put(key, &placeholder); err != nil {
		return "", err
	}
	return parentURI, nil
}

func (s *store) allRecordKeys(ctx context.Context) ([]recordKeyModel, []*gcdatastore.Key, error) {
	var recordKeys []recordKeyModel
	keys, err := s.client.GetAll(ctx, s.query(kindRecordKey), &recordKeys)
	if err != nil {
		return nil, nil, err
	}
	return recordKeys, keys, nil
}
