...
func NewRecordRepository(client *gcdatastore.Client, namespace string) usecase.RecordRepository {
...
func (r *RecordRepository) CreateRecord(ctx context.Context, write usecase.RecordWrite) (string, error) {
	ctx, span := tracer.Start(ctx, "Repository.Datastore.Record.CreateRecord")
	defer span.End()
	...
	_, err := r.client.RunInTransaction(ctx, func(tx *gcdatastore.Transaction) error {
		...
		if _, err := tx.Put(r.key(kindRecord, write.DocumentID), &record); err != nil {
			return err
		}

		var oldRecordKey recordKeyModel
		oldRecordID := ""
		recordKey := r.key(kindRecordKey, write.Key)
		err = tx.Get(recordKey, &oldRecordKey)
		if err == nil {
		...
		if _, err := tx.Put(recordKey, &newRecordKey); err != nil {
			return err
		}

		if oldRecordID != "" && oldRecordID != write.DocumentID {
			var oldCommit commitLogModel
			oldCommitKey := r.key(kindCommitLog, oldRecordID)
			err := tx.Get(oldCommitKey, &oldCommit)
			if err == nil {
				oldCommit.GcCandidate = true
				if _, err := tx.Put(oldCommitKey, &oldCommit); err != nil {
					return err
				}
			} else if !isNoSuchEntity(err) {
				return err
			}
			if err := tx.Delete(r.key(kindRecord, oldRecordID)); err != nil {
				return err
			}
		}
...
func (r *RecordRepository) CreateAssociation(ctx context.Context, write usecase.AssociationWrite) error {
	ctx, span := tracer.Start(ctx, "Repository.Datastore.Record.CreateAssociation")
	defer span.End()
	...
	_, err := r.client.RunInTransaction(ctx, func(tx *gcdatastore.Transaction) error {
		...
		_, err := tx.Put(r.key(kindAssociation, write.Unique), &association)
		return err
	})
	if err != nil {
		span.RecordError(err)
...
func (r *RecordRepository) Acknowledge(ctx context.Context, write usecase.AckWrite) (string, error) {
	ctx, span := tracer.Start(ctx, "Repository.Datastore.Record.Acknowledge")
	defer span.End()
...
func (r *RecordRepository) UnAcknowledge(ctx context.Context, write usecase.AckWrite) error {
	ctx, span := tracer.Start(ctx, "Repository.Datastore.Record.UnAcknowledge")
	defer span.End()
...
func (r *RecordRepository) saveAck(ctx context.Context, write usecase.AckWrite) error {
	_, err := r.client.RunInTransaction(ctx, func(tx *gcdatastore.Transaction) error {
		...
		key := r.key(kindAck, compoundKey(write.From, write.To, write.Context))
		ack := ackModel{
			From:       write.From,
			To:         write.To,
...
func (r *RecordRepository) GetSignedDocument(ctx context.Context, uri string) (*concrnt.SignedDocument, error) {
	ctx, span := tracer.Start(ctx, "Repository.Datastore.Record.GetSignedDocument")
	defer span.End()
...
func (r *RecordRepository) GetHierarchicalRecordPolicies(ctx context.Context, uri string) ([]concrnt.Policy, error) {
	ctx, span := tracer.Start(ctx, "Repository.Datastore.Record.GetHierarchicalRecordPolicies")
	defer span.End()
...
func (r *RecordRepository) GetDistributions(ctx context.Context, uri string) ([]string, error) {
	ctx, span := tracer.Start(ctx, "Repository.Datastore.Record.GetDistributions")
	defer span.End()
...
func (r *RecordRepository) Delete(ctx context.Context, sd concrnt.SignedDocument) (string, error) {
	ctx, span := tracer.Start(ctx, "Repository.Datastore.Record.Delete")
	defer span.End()
...
func (r *RecordRepository) GetAssociatedRecords(ctx context.Context, targetURI, schema, variant, author string) ([]concrnt.SignedDocument, error) {
	ctx, span := tracer.Start(ctx, "Repository.Datastore.Record.GetAssociatedRecords")
	defer span.End()
...
func (r *RecordRepository) GetAssociatedRecordCountsBySchema(ctx context.Context, targetURI string) (map[string]int64, error) {
	ctx, span := tracer.Start(ctx, "Repository.Datastore.Record.GetAssociatedRecordCountsBySchema")
	defer span.End()
...
func (r *RecordRepository) GetAssociatedRecordCountsByVariant(ctx context.Context, targetURI, schema string) (*utils.OrderedKVMap[int64], error) {
	ctx, span := tracer.Start(ctx, "Repository.Datastore.Record.GetAssociatedRecordCountsByVariant")
	defer span.End()
...
func (r *RecordRepository) QueryByPrefix(ctx context.Context, prefix, schema string, since, until *time.Time, limit int, order string) ([]concrnt.SignedDocument, error) {
	ctx, span := tracer.Start(ctx, "Repository.Datastore.Record.QueryByPrefix")
	defer span.End()

	var recordKeys []recordKeyModel
	keys, err := r.client.GetAll(ctx, r.query(kindRecordKey).Filter("prefixes =", prefix), &recordKeys)
	if err != nil {
		span.RecordError(err)
		return nil, err
	}
...
func (r *RecordRepository) QueryByParent(ctx context.Context, parent, schema string, since, until *time.Time, limit int, order string) ([]concrnt.SignedDocument, error) {
	ctx, span := tracer.Start(ctx, "Repository.Datastore.Record.QueryByParent")
	defer span.End()

	var recordKeys []recordKeyModel
	_, err := r.client.GetAll(ctx, r.query(kindRecordKey).Filter("parentURI =", parent), &recordKeys)
	if err != nil {
		span.RecordError(err)
		return nil, err
	}
...
func (r *RecordRepository) GetAcknowledgeRecords(ctx context.Context, from, to, contextName string) ([]concrnt.SignedDocument, error) {
	ctx, span := tracer.Start(ctx, "Repository.Datastore.Record.GetAcknowledgeRecords")
	defer span.End()
...
func (r *RecordRepository) GetAcknowledgeRecordCounts(ctx context.Context, from, to, contextName string) (map[string]int64, error) {
	ctx, span := tracer.Start(ctx, "Repository.Datastore.Record.GetAcknowledgeRecordCounts")
	defer span.End()
...
func (r *RecordRepository) GetAllCommitLogs(ctx context.Context, owner string) ([]concrnt.SignedDocument, error) {
	ctx, span := tracer.Start(ctx, "Repository.Datastore.Record.GetAllCommitLogs")
	defer span.End()

	var commitOwners []commitOwnerModel
	_, err := r.client.GetAll(ctx, r.query(kindCommitOwner).Filter("owner =", owner), &commitOwners)
	if err != nil {
		span.RecordError(err)
		return nil, err
	}
...
func (r *RecordRepository) allAssociations(ctx context.Context) ([]associationModel, []*gcdatastore.Key, error) {
	var associations []associationModel
	keys, err := r.client.GetAll(ctx, r.query(kindAssociation), &associations)
	if err != nil {
		return nil, nil, err
	}
	return associations, keys, nil
}

func (r *RecordRepository) allAcks(ctx context.Context) ([]ackModel, []*gcdatastore.Key, error) {
	var acks []ackModel
	keys, err := r.client.GetAll(ctx, r.query(kindAck), &acks)
	if err != nil {
		return nil, nil, err
	}
	return acks, keys, nil
}
...
func (r *RecordRepository) deleteCommitReadModels(ctx context.Context, commitID string, targetURI *string) error {
	...
	var commitOwners []commitOwnerModel
	commitOwnerKeys, err := r.client.GetAll(ctx, r.query(kindCommitOwner), &commitOwners)
	if err != nil {
		return err
	}

	_, err = r.client.RunInTransaction(ctx, func(tx *gcdatastore.Transaction) error {
		if err := tx.Delete(r.key(kindCommitLog, commitID)); err != nil {
			return err
		}
		if err := tx.Delete(r.key(kindRecord, commitID)); err != nil {
			return err
		}
		...
		for i, rk := range recordKeys {
			...
			rk.Redirect = ""
			if _, err := tx.Put(recordKeyKeys[i], &rk); err != nil {
				return err
			}
		}
