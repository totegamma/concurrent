...
func NewServerRepository(config *domain.Config, ds *gcdatastore.Client, namespace string, cl *client.Client) usecase.ServerRepository {
...
func (r *ServerRepository) Resolve(ctx context.Context, identifier string, hint *string) (*domain.Server, error) {
	ctx, span := tracer.Start(ctx, "Repository.Datastore.Server.Resolve")
	defer span.End()
...
func (r *ServerRepository) getAndCacheByCSID(ctx context.Context, csid string, hint *string) (*domain.Server, error) {
	...
	_, err := r.client.GetAll(ctx, r.query(kindServer).Filter("csid =", csid).Limit(1), &servers)
	if err != nil {
		return nil, err
	}
	...
	if _, err := r.client.Put(ctx, r.key(kindServer, wkc.Domain), &model); err != nil {
		return nil, err
	}
...
func (r *ServerRepository) getAndCacheByFQDN(ctx context.Context, fqdn string) (*domain.Server, error) {
	...
	err := r.client.Get(ctx, r.key(kindServer, fqdn), &model)
...
func (r *ServerRepository) List(ctx context.Context) ([]*concrnt.WellKnownConcrnt, error) {
	ctx, span := tracer.Start(ctx, "Repository.Datastore.Server.List")
	defer span.End()

	var servers []serverModel
	_, err := r.client.GetAll(ctx, r.query(kindServer), &servers)
	if err != nil {
