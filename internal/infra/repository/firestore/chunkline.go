...
func NewChunklineRepository(client *gcfirestore.Client, namespace string) usecase.ChunklineRepository {
...
func (r *ChunklineRepository) GetChunklineManifest(ctx context.Context, uri string) (*chunkline.Manifest, error) {
	ctx, span := tracer.Start(ctx, "Repository.Firestore.Chunkline.GetChunklineManifest")
	defer span.End()
...
func (r *ChunklineRepository) LookupLocalItrs(ctx context.Context, uris []string, chunkID int64) (map[string]int64, error) {
	ctx, span := tracer.Start(ctx, "Repository.Firestore.Chunkline.LookupLocalItrs")
	defer span.End()
...
func (r *ChunklineRepository) LoadLocalBody(ctx context.Context, uri string, chunkID int64) ([]chunkline.BodyItem, error) {
	ctx, span := tracer.Start(ctx, "Repository.Firestore.Chunkline.LoadLocalBody")
	defer span.End()
...
func (r *ChunklineRepository) children(ctx context.Context, parent string) ([]recordKeyModel, error) {
	children, _, err := getAll[recordKeyModel](ctx, r.query(kindRecordKey).Where("parentURI", "==", parent))
	if err != nil {
		return nil, err
	}
