...
func NewEntityRepository(ds *gcfirestore.Client, namespace string, cl *client.Client, config domain.Config) usecase.EntityRepository {
...
func (r *EntityRepository) SaveMeta(ctx context.Context, meta domain.EntityMeta) error {
	ctx, span := tracer.Start(ctx, "Repository.Firestore.Entity.SaveMeta")
	defer span.End()

	ref := r.doc(kindEntityMeta, meta.ID)
	now := time.Now().UTC()
...
func (r *EntityRepository) SaveEntity(ctx context.Context, sd concrnt.SignedDocument) (*concrnt.Document[schemas.Entity], error) {
	ctx, span := tracer.Start(ctx, "Repository.Firestore.Entity.SaveEntity")
	defer span.End()
	...
	if entity.Value.Domain == r.config.FQDN {
		...
		err := r.get(ctx, r.doc(kindEntityMeta, entity.Author), &meta)
		if err != nil {
	...
	err = r.client.RunTransaction(ctx, func(ctx context.Context, tx *gcfirestore.Transaction) error {
		...
		ref := r.doc(kindEntity, entity.Author)
...
func (r *EntityRepository) Get(ctx context.Context, ccid string, hint *string) (*domain.Entity, error) {
	ctx, span := tracer.Start(ctx, "Repository.Firestore.Entity.Get")
	defer span.End()

	var model entityModel
	err := r.get(ctx, r.doc(kindEntity, ccid), &model)
	if err == nil {
...
func (r *EntityRepository) GetByAlias(ctx context.Context, alias string) (*domain.Entity, error) {
	ctx, span := tracer.Start(ctx, "Repository.Firestore.Entity.GetByAlias")
	defer span.End()

	alias = strings.TrimPrefix(alias, "@")
	models, _, err := getAll[entityModel](ctx, r.query(kindEntity).Where("alias", "==", alias).Limit(1))
	if err != nil {
		span.RecordError(err)
		return nil, err
	}
...
func (r *EntityRepository) GetSD(ctx context.Context, ccid string, hint *string) (*concrnt.SignedDocument, error) {
	ctx, span := tracer.Start(ctx, "Repository.Firestore.Entity.GetSD")
	defer span.End()
...
func (r *EntityRepository) GetDocument(ctx context.Context, ccid string, hint *string) (*concrnt.Document[schemas.Entity], error) {
	ctx, span := tracer.Start(ctx, "Repository.Firestore.Entity.GetDocument")
	defer span.End()
...
func (r *EntityRepository) GetMeta(ctx context.Context, ccid string) (*domain.EntityMeta, error) {
	ctx, span := tracer.Start(ctx, "Repository.Firestore.Entity.GetMeta")
	defer span.End()

	var model entityMetaModel
	err := r.get(ctx, r.doc(kindEntityMeta, ccid), &model)
	if err != nil {
		if isNoSuchEntity(err) {
			return nil, domain.NotFoundError{Resource: ccid}
		}
