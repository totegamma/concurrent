...
func TestRecordRepositoryFirestoreIntegration(t *testing.T) {
	if os.Getenv("FIRESTORE_EMULATOR_HOST") == "" || os.Getenv("FIRESTORE_PROJECT_ID") == "" {
		t.Skip("FIRESTORE_EMULATOR_HOST and FIRESTORE_PROJECT_ID are required")
	}

	ctx := context.Background()
	client, err := NewClient(ctx, os.Getenv("FIRESTORE_PROJECT_ID"), "")
	...
	oldSD := firestoreSignedDocument(t, concrnt.Document[map[string]string]{
		Key:       key,
	...
	newSD := firestoreSignedDocument(t, concrnt.Document[map[string]string]{
		Key:       key,
	...
	childSD := firestoreSignedDocument(t, concrnt.Document[map[string]string]{
		Key:       childKey,
	...
	variant := "reply"
	associationSD := firestoreSignedDocument(t, concrnt.Document[map[string]string]{
	...
	ackSD := firestoreSignedDocument(t, concrnt.Document[schemas.Acknowledge]{
...
func firestoreSignedDocument(t *testing.T, doc any) concrnt.SignedDocument {
	t.Helper()
